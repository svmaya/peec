    ...
    main.innerHTML = `
      ...
      <div class="content fade">
        ...
        <div class="card" style="position:relative">
          ...
          ${groups.length ? groups.map(g => `<div class="grp"><div class="gh"><span class="tri">▾</span><span>${esc(g.title)}</span><span class="cnt">${g.items.length}</span>${g.title === "Content optimization" ? '<span class="tag ill" style="margin-left:8px" title="Peec has this action type; the Nike snapshot had none, so these 7 are invented for the demo">Illustrative</span>' : ""}</div>${g.items.map(it => `
